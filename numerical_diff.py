...
def _auto_step_second_order(
    ...
    for h in h_candidates:
        try:
            d = _cd2_core(f, x, h)
            results.append((h, float(d)))
        except (ZeroDivisionError, FloatingPointError, ValueError):
            continue

    if len(results) < 2:
        h = h_candidates[len(h_candidates) // 2]
        return float(_cd2_core(f, x, h)), h
...
class NumericalDifferentiator:
    ...
    def second_derivative(
        ...
        if self.h == 'auto':
            d, best_h = _auto_step_second_order(f, x)
            return (d, best_h) if return_h else d
        else:
            d = _cd2_core(f, x, self.h)
            return (float(d), self.h) if return_h else float(d)
